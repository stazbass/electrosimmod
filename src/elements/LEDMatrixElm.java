...
public class LEDMatrixElm extends ChipElm {
	...
	public void draw(Graphics g) {
		...
		for (int col = 0; col < size; col++)
			for (int row = 0; row < size; row++) {
				int centreX = getX() + 2 * (col + 1) * cspc;
				int centreY = getY() + 2 * row * cspc;
				int radius = cspc / 2;
	...
	public EditInfo getEditInfo(int n) {
		if (n == 2) {
			...
			ei.setCheckbox(new Checkbox("Negate rows", negateRows));
			return ei;
		}
		if (n == 3) {
			EditInfo ei = new EditInfo("", 0, -1, -1);
			ei.setCheckbox(new Checkbox("Negate columns", negateColumns));
			return ei;
		}
		if (n == 4) {
