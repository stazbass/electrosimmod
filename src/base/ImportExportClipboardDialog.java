...
public class ImportExportClipboardDialog extends Dialog implements ImportExportDialog, ActionListener {
	...
	ImportExportClipboardDialog(CirSim f, Action type) {
		...
		/*
		 * if (type == Action.EXPORT) importButton = new Button("Copy to clipboard");
		 * else
		 */
		...
		Point x = cframe.getMain().getLocationOnScreen();
		resize(400, 300);
		Dimension d = getSize();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == importButton) {
			/*
			 * if (clipboard == null) clipboard = getToolkit().getSystemClipboard(); if (
			 * type == Action.EXPORT ) { StringSelection data = new
			 * StringSelection(text.getText()); clipboard.setContents(data, data); } else
			 */
			{
