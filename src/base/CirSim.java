...
import elements.CapacitorElm;
import elements.CircuitElm;
import elements.CurrentElm;
import elements.GraphicElm;
import elements.GroundElm;
import elements.InductorElm;
import elements.RailElement;
import elements.ResistorElm;
import elements.SwitchElm;
import elements.VoltageElement;
import elements.WireElm;
...
//	setupList = new Vector();
	...
	public void updateCircuit(Graphics realg) {
		...
		if (editDialog != null && editDialog.elm instanceof CircuitElm)
			setMouseElm((CircuitElm) (editDialog.elm));
		...
		if (getNodeList() != null)
			for (i = 0; i != getNodeList().size(); i++) {
				...
				if (!cn.internal && cn.getLinks().size() == 1) {
					int bb = 0, j;
					CircuitNodeLink cnl = cn.getLinks().elementAt(0);
					for (j = 0; j != getElmList().size(); j++) { // TODO: (hausen) see if this change does not break stuff
	...
	void analyzeCircuit() {
		...
		for (i = 0; i != getElmList().size(); i++) {
			...
			for (j = 0; j != inodes; j++) {
				...
				cnl.elm = ce;
				cn.getLinks().addElement(cnl);
				ce.setNode(cnl.num, getNodeList().size());
				getNodeList().addElement(cn);
			}
			vscount += ivs;
		}
		...
		for (i = 0; i != getElmList().size(); i++) {
			...
			if ((ce instanceof VoltageElement && ce.getPostCount() == 2) || ce instanceof WireElm) {
		...
		boolean findPath(int n1, int depth) {
			...
			for (i = 0; i != getElmList().size(); i++) {
				...
				if (type == SHORT && !ce.isWire())
					continue;
				if (type == CAP_V) {
					if (!(ce.isWire() || ce instanceof CapacitorElm || ce instanceof VoltageElement))
						continue;
				}
				if (n1 == 0) {
				...
				for (j = 0; j != circuitMatrixFullSize; j++) {
					...
					if (j < getNodeList().size() - 1) {
						...
						for (k = 0; k != cn.getLinks().size(); k++) {
							CircuitNodeLink cnl = (CircuitNodeLink) cn.getLinks().elementAt(k);
...
//		 ImportExportDialog.Action.EXPORT);
	...
	public void dragPost(int x, int y) {
		if (draggingPost == -1) {
			draggingPost = (distanceSq(getMouseElm().getX(), getMouseElm().getY(), x, y) > distanceSq(getMouseElm().getX2(), getMouseElm().getY2(), x, y)) ? 1
					: 0;
		}
		int dx = x - dragX;
		int dy = y - dragY;
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (getDragElm() != null) {
			...
			if (getDragElm().getX() == getDragElm().getX2() && getDragElm().getY() == getDragElm().getY2())
				...
				getElmList().addElement(getDragElm());
				circuitChanged = true;
			}
			setDragElm(null);
		}
